...
and expr_type e env = 
  match e.e_guts with
      Variable x -> 
        lookup_def x env
    | Sub (v, e) ->
        let checkv = check_expr v env in
        let checke = check_expr e env in
        (match checkv with
            Array (_,_) -> if checke == Integer then base_type (expr_type v env) else type_error ();
        |   _ -> type_error ();)
...
let rec check_stmt s env =
  match s with
    ...
    | Assign (lhs, rhs) ->
        ...
        if ta <> tb then sem_error "type mismatch in assignment: type of lhs != type of rhs" [] 
        else if not (((ta == Integer) && (tb == Integer)) || ((ta == Boolean) && (tb == Boolean))) then sem_error "type error in assignment" [];
    | Print e ->
