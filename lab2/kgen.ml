...
let rec gen_expr e =
  match e.e_guts with
      Variable x ->
      let d = get_def x in
      if is_base_bool d.d_type then
          SEQ [gen_addr e; LOADC]
      else
          SEQ [gen_addr e; LOADW]   
    | Sub (x,_) ->
        if is_base_bool e.e_type then
            SEQ [gen_addr e; LOADC]
        else
            SEQ [gen_addr e; LOADW] 
    | Constant (n, t) ->
...
(* |is_base_bool| -- returns true if the base type of the (multidimensional) array a Boolean *)
and is_base_bool x =
    if is_array x then
        is_base_bool (base_type x)
    else
        x == Boolean
...
and gen_addr v =
  match v.e_guts with
      Sub (x,e) ->
        let xtype = base_type x.e_type in
        let size = type_size xtype in
        SEQ [gen_addr x; gen_expr e; CONST size; BINOP Times; OFFSET]  
    | Variable x ->
        let d = get_def x in
