...
let rec repeat_list n xs =
    if n < 1 then []
    else
        xs @ (repeat_list (n-1) xs)
    ...
    (match d.d_kind with
          VarDef ->
            if n = 0 then
              ...
              SEQ [LOCAL 0; SEQ (repeat_list (m-n) [CONST 12; OFFSET; LOADW]); CONST d.d_off; OFFSET]
        | ProcDef _ ->
            GLOBAL d.d_lab)
    (* let k = 
        (match d.d_kind with
            ...
            let n' = n - k in
                if n' = 0 then
                    ...
                    if d.d_off = 0 then 
                        failwith (String.concat "" ["offset incorrectly = 0; tag="; d.d_tag; " nestinglevel="; string_of_int d.d_level; " label="; d.d_lab; " offset="; string_of_int d.d_off])
                    else
                        SEQ [LOCAL 0; SEQ (repeat_list (n'-m) [CONST 12; OFFSET; LOADW]); CONST d.d_off; OFFSET] *)
    ...
    | Call (p, args) ->
        ...
        match y with
              Some d ->
                let n = d.d_level in
                let m = !level in    
                    SEQ [LINE p.x_line; SEQ (List.map gen_expr (List.rev args)); LOCAL 0; SEQ (repeat_list ((m - n) + 1) [CONST 12; OFFSET; LOADW]); gen_addr d; PCALLW (List.length args)]
            | None -> 
                failwith "procedure definition not found"
