...
let rec gen_stmt s exit_lab =
  match s with
      Skip -> NOP
    | Seq stmts -> 
        let func x = gen_stmt x exit_lab in 
        SEQ (List.map func stmts)
    | Assign (v, e) ->
    ...
    | RepeatStmt (test, body) ->
        ...
        SEQ [LABEL lab1; gen_stmt body exit_lab; gen_cond test lab2 lab1; LABEL lab2]
    | LoopStmt (body) ->
        let lab1 = label () and lab2 = label () in
...
let translate (Program ss) =
  let lab0 = label () in
  let code = SEQ [gen_stmt ss lab0; LABEL lab0] in
